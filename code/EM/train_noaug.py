...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        val_acc, val_acc1, val_acc2, val_acc3, val_acc4, val_f1, val_f11, val_f12, val_f13, val_f14 = test_all_set(
            val_tune_loader, model)
        '''
        ...
        if val_acc >= best_acc:
            ...
            all_acc, all_acc1, all_acc2, all_acc3, all_acc4, all_f1, all_f11, all_f12, all_f13, all_f14 = test_all_set(
                test_tune_loader, model)
            all_test_accs.append(all_acc)
            all_test_f1s.append(all_f1)
            all_test_accs1.append(all_acc1)
            all_test_f1s1.append(all_f11)
            all_test_accs2.append(all_acc2)
            all_test_f1s2.append(all_f12)
            all_test_accs3.append(all_acc3)
            all_test_f1s3.append(all_f13)
            all_test_accs4.append(all_acc4)
            all_test_f1s4.append(all_f14)
            ...
            print("epoch {}, all test acc2 {}".format(epoch, all_acc2))
            print("epoch {}, all f12 {}".format(epoch, all_f12))
            print("epoch {}, all test acc3 {}".format(epoch, all_acc3))
            print("epoch {}, all f13 {}".format(epoch, all_f13))
            print("epoch {}, all test acc4 {}".format(epoch, all_acc4))
            print("epoch {}, all f14 {}".format(epoch, all_f14))
        ...
        print('All test acc2:')
        print(all_test_accs2)

        print('All test f12:')
        print(all_test_f1s2)

        print('All test acc3:')
        print(all_test_accs3)

        print('All test f13:')
        print(all_test_f1s3)

        print('All test acc4:')
        print(all_test_accs4)
...
def test_all_set(testloader, model):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets, targets_tune, length) in enumerate(testloader):
            ...
            predicted2 = torch.clone(predicted)
            predicted3 = torch.clone(predicted)
            predicted4 = torch.clone(predicted)
            predicted5 = torch.clone(predicted)

            clf = LocalOutlierFactor()
            predicted_out = clf.fit_predict(embedding.cpu())
            clf1 = LocalOutlierFactor(n_neighbors=5, leaf_size=10)
            clf2 = LocalOutlierFactor(n_neighbors=2, leaf_size=10)
            clf3 = LocalOutlierFactor(n_neighbors=30, leaf_size=30)
            clf4 = LocalOutlierFactor(n_neighbors=40, leaf_size=40)
            predicted1_out = clf1.fit_predict(embedding.cpu())
            predicted2_out = clf2.fit_predict(embedding.cpu())
            predicted3_out = clf3.fit_predict(embedding.cpu())
            predicted4_out = clf4.fit_predict(embedding.cpu())

            for idx, _ in enumerate(predicted_out):
                if predicted_out[idx] != 1:  # predicted_out
                    predicted[idx] = -1
                if predicted1_out[idx] != 1:
                    predicted1[idx] = -1
                if predicted2_out[idx] != 1:
                    predicted2[idx] = -1
                if predicted2_out[idx] != 1:
                    predicted2[idx] = -1
                if predicted3_out[idx] != 1:
                    predicted3[idx] = -1
                if predicted4_out[idx] != 1:
                    predicted4[idx] = -1
            ...
            correct1 += (np.array(predicted1.cpu()) ==
                         np.array(targets.cpu())).sum()
            correct2 += (np.array(predicted2.cpu()) ==
                         np.array(targets.cpu())).sum()
            correct3 += (np.array(predicted3.cpu()) ==
                         np.array(targets.cpu())).sum()
            correct4 += (np.array(predicted4.cpu()) ==
                         np.array(targets.cpu())).sum()
            total_sample += inputs.shape[0]
            pred_list.extend(np.array(predicted.cpu()))
            target_list.extend(np.array(targets.cpu()))
            pred_list1.extend(np.array(predicted1.cpu()))
            target_list1.extend(np.array(targets.cpu()))
            pred_list2.extend(np.array(predicted2.cpu()))
            target_list2.extend(np.array(targets.cpu()))
            pred_list3.extend(np.array(predicted3.cpu()))
            target_list3.extend(np.array(targets.cpu()))
            pred_list4.extend(np.array(predicted4.cpu()))
            target_list4.extend(np.array(targets.cpu()))

        acc_total = correct / total_sample
        f1 = f1_score(target_list, pred_list, average='macro')
        f11 = f1_score(target_list, pred_list1, average='macro')
        f12 = f1_score(target_list, pred_list2, average='macro')
        f13 = f1_score(target_list, pred_list3, average='macro')
        f14 = f1_score(target_list, pred_list4, average='macro')
        ...
        accuracy1 = accuracy_score(target_list, pred_list1)
        accuracy2 = accuracy_score(target_list, pred_list2)
        accuracy3 = accuracy_score(target_list, pred_list3)
        accuracy4 = accuracy_score(target_list, pred_list4)

    return accuracy, accuracy1, accuracy2, accuracy3, accuracy4, f1, f11, f12, f13, f14
