...
def get_data_cls_extreme_tune(data_path, extreme_data_path, n_labeled_per_class, unlabeled_per_class=5000, max_seq_len=256,
                      model='bert-base-uncased',
                      train_aug=False, seed=0, use_labels=False, cls="MSP"):
    """Read data, split the dataset, and build dataset for dataloaders.
    ...
    choose_train_extreme_unlabeled = []
    for i in tqdm(range(len(train_extreme_text))):
        result, length = loader.get_tokenized(train_extreme_text[i])
        result = torch.tensor(result).unsqueeze(0).cuda()
        emb = cls_encoder(torch.tensor(result))
        ...
        if cls == 'LMCL':
            ...
            outlier, confidence_score = classifier.predict(emb, index_select, threshold=threshold)
        if outlier.item() == 1:
            choose_train_extreme_unlabeled.append(train_extreme_text[i])
            if -1 not in train_labels_map.keys():
                train_labels_map[-1] = 1
            else:
                train_labels_map[-1] += 1
    choose_train_extreme_unlabeled = np.array(choose_train_extreme_unlabeled)
    ...
    if use_labels:
        ...
        train_unlabeled_dataset = loader_unlabeled_extreme(
            train_text[choose_train_unlabeled_idxs], choose_train_unlabeled_idxs, tokenizer, max_seq_len, choose_train_extreme_unlabeled)
    ...
    print("#Labeled: {}, Unlabeled {}, Val {}, Test {}".format(len(
        train_labels[in_distribution_train_labeled_idxs]),
        len(train_labels[train_unlabeled_idxs]) + len(train_extreme_text),
