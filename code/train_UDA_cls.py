...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        '''
        val_acc, val_f1, val_roc_auc, val_acc_lof, val_f1_lof = test_all_set(val_tune_loader, model, encoder,
                                                                             classifier, criterion, index_select)
        ...
        if val_acc >= best_acc:
            ...
            out_test_acc, out_test_acc_lof = test_out_of_distribution_set(test_tune_loader, model, encoder,
                                                                          classifier, criterion,
                                                                          index_select)
            out_distribution_test_accs.append(out_test_acc)
            out_distribution_test_accs_lof.append(out_test_acc_lof)
            all_acc, all_f1, all_roc_auc, all_acc_lof, all_f1_lof = test_all_set(test_tune_loader, model, encoder,
                                                                                 classifier, criterion,
                                                                                 index_select)
