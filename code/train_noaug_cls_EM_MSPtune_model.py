...
def main():
    ...
    for ((inputs_u, inputs_u2, inputs_ori), (length_u, length_u2, length_ori)) in unlabeled_trainloader:
        ...
        emb = encoder(inputs_ori)
        predicted_out, confidence_score = classifier.predict(emb, index_select)

        for i in range(inputs_ori.size(0)):
            if predicted_out[i] == 1:
                Q_0[str(inputs_ori[i])] = 1 - confidence_score[i]
                Q_1[str(inputs_ori[i])] = confidence_score[i]
            else:
                Q_0[str(inputs_ori[i])] = confidence_score[i]
                Q_1[str(inputs_ori[i])] = 1 - confidence_score[i]
    ...
    for epoch in range(args.epochs):
        ...
        val_acc, val_f1, val_roc_auc = test_all_set(val_tune_loader, model)
        ...
        if val_acc >= best_acc:
            ...
            out_test_acc = test_out_of_distribution_set(out_distribution_test_loader, model)
            out_distribution_test_accs.append(out_test_acc)
            all_acc, all_f1, all_roc_auc = test_all_set(test_tune_loader, model)
