...
def main():
    ...
    print(
        "This is epoch {}, The Q_1_in mean is {}, the Q_1_out mean is {}, the Q_0_in mean is {}, the Q_0_out_mean is {}"
        .format(0, 0.5, 0.5, 0.5, 0.5))
    ...
    for epoch in range(args.epochs):
        ...
        if epoch > 0:
            ...
            with torch.no_grad():
                ...
                for batch_idx, (inputs, targets, length) in enumerate(unlabeled_labeled_trainloader):
                    inputs, targets = inputs.cuda(), targets.cuda(non_blocking=True)
                    outputs, _ = model(inputs)
                    pred = torch.softmax(outputs, dim=1)
                    entropy_0, entropy_1 = normalize_entropy_Q(pred, lam=1.25)
                    Q_0_out += entropy_0[torch.where(targets.eq(-1))].sum()
                    Q_1_out += entropy_1[torch.where(targets.eq(-1))].sum()
                    Q_0_in += entropy_0[torch.where(targets.ne(-1))].sum()
                    Q_1_in += entropy_1[torch.where(targets.ne(-1))].sum()
                    all_sum_out += entropy_0[torch.where(targets.eq(-1))].size(0)
                    all_sum_in += entropy_1[torch.where(targets.ne(-1))].size(0)

                print(
                    "This is epoch {}, The Q_1_in mean is {}, the Q_1_out mean is {}, the Q_0_in mean is {}, the Q_0_out_mean is {}"
                    .format(epoch, Q_1_in / all_sum_in, Q_1_out / all_sum_out, Q_0_in / all_sum_in,
                            Q_0_out / all_sum_out))
        ...
        if val_acc >= best_acc:
            ...
            print("In the epoch {}, get a better val acc".format(record))
            counter = 0
            best_acc = val_acc
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets, targets_tune, length) in enumerate(testloader):
            ...
            correct += (np.array(pred_1.cpu()) ==
                        np.array(targets_tune.cpu())).sum()  # 在这里的时候，targets_tune中的元素都是0或者1，需要在下面换成-1和1
