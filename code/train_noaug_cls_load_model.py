...
def main():
    ...
    data_set_name = "agnews"
    model = nn.DataParallel(model)
    # model.cuda()
    model_dict_temp = torch.load(args.data_path + 'model_{}_n_labels_{}.pt'.format(data_set_name, args.n_labeled),
                                 map_location={'cuda:0': 'cuda:1'})
    model_dict = {}
    for key_i in model_dict_temp.keys():
        model_dict['module.' + key_i] = model_dict_temp[key_i]
    ...
    for epoch in range(args.epochs):
        ...
        for ((inputs_u, inputs_u2, inputs_ori), (length_u, length_u2, length_ori)) in unlabeled_trainloader:
            ...
            entropy_u_0, entropy_u_1 = normalize_entropy_Q(pred_u)
            # entropy_u2_0, entropy_u2_1 = normalize_entropy_Q(pred_u2)
            # entropy_ori_0, entropy_ori_1 = normalize_entropy_Q(pred_ori)

            for i in range(inputs_u.size(0)):
                Q_0[str(inputs_u[i])] = entropy_u_0[i]
                Q_1[str(inputs_u[i])] = entropy_u_1[i]

        train(labeled_trainloader, unlabeled_trainloader, model, optimizer,
              scheduler, train_criterion, epoch, n_labels, Q_0, Q_1, args.train_aug)
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, scheduler, criterion, epoch, n_labels, Q_0, Q_1,
          train_aug=False):
    ...
    for batch_idx in range(args.val_iteration):
        ...
        if train_aug:
            ...
            outputs_u = model(inputs_u)
            pred_u = torch.softmax(outputs_u, dim=1)
            P_u_0, P_u_1 = normalize_entropy_Q(pred_u)
            for i in range(inputs_u.size(0)):
                loss_reg += (Q_0[str(inputs_u[i])] * torch.log2(P_u_0[i]))
                loss_reg += (Q_1[str(inputs_u[i])] * torch.log2(P_u_1[i]))
...
def validate(valloader, model, criterion, epoch, mode):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets, length) in enumerate(valloader):
            ...
            pred_0, pred_1 = normalize_entropy_Q(outputs)

            for idx, data in pred_1:
                if data > 0.5:
                    if targets[idx] in in_distribution_map:
                        in_distribution_map[targets[idx]] += 1
                    else:
                        in_distribution_map[targets[idx]] = 0
...
def normalize_entropy_Q(probility):  # [batch_size, num_labels]
    # 将概率中的所有0全部换成1，算出来的结果是一样的，至少我这么觉得
    for i in range(probility.size(0)):
        for j in range(probility.size(1)):
            if probility[i][j] == 0:
                probility[i][j] = 1
